...
(() => {
    ...
    const avatarTrigger = $('#avatar-button');
    ...
    const defaultBackgrounds = [
        ...
        { id: 'florecita', name: 'Florecita', file: 'florecita.gif', isDefault: true },
        { id: 'chillmode', name: 'Modo Chill', file: 'chillmode.gif', isDefault: true }
    ];
    ...
    function makeDraggable(panel) {
        ...
        if (panelPositions[id]) {
            ...
            // Ensure restored positions do not overlap sidebar or topbar
            ...
            if (sidebarEl) {
                ...
                minLeft = Math.max(0, sidebarRect.right - parentRect.left + SIDE_MARGIN);
            }

            let minTop = 0;
            if (topbarEl) {
                const topbarRect = topbarEl.getBoundingClientRect();
                minTop = Math.max(0, topbarRect.bottom - parentRect.top + TOP_MARGIN);
            }

            const maxLeft = Math.max(minLeft, parentRect.width - panel.offsetWidth);
            const maxTop = Math.max(minTop, parentRect.height - panel.offsetHeight);

            const safeLeft = clamp(pos.x, minLeft, maxLeft);
            const safeTop = clamp(pos.y, minTop, maxTop);
        ...
        const onPointerDown = (ev) => {
            ...
            // bring to front
            // ensure the dragged panel is above the sidebar and other floating panels
            let maxZ = 15;
            floatingPanels.forEach(p => {
                const z = parseInt(window.getComputedStyle(p).zIndex) || 15;
                if (z > maxZ) maxZ = z;
            });
            // check sidebar z-index (if present) and ensure we are above it
            const sidebarEl = document.querySelector('.sidebar');
            const sidebarZ = sidebarEl ? (parseInt(window.getComputedStyle(sidebarEl).zIndex) || 20) : 20;
            const MIN_TOP_Z = 51; // keep above sidebar (20) and default panels (15)
            const newZ = Math.max(maxZ + 1, sidebarZ + 1, MIN_TOP_Z);
        ...
        const onPointerMove = (ev) => {
            ...
            // Prevent panel from touching or overlapping the sidebar (functions area)
            const sidebarEl = document.querySelector('.sidebar');
            const topbarEl = document.querySelector('.topbar');
            const SIDE_MARGIN = 8; // px gap to keep from touching the sidebar
            const TOP_MARGIN = 8; // px gap to keep from touching the topbar
            let minLeft = 0;
            if (sidebarEl) {
                const sidebarRect = sidebarEl.getBoundingClientRect();
                // Compute the minimum left position relative to parent to avoid overlapping sidebar
                minLeft = Math.max(0, sidebarRect.right - parentRect.left + SIDE_MARGIN);
            }
            ...
            const newLeft = clamp(origin.x + dx - parentRect.left, minLeft, maxLeft);
            const newTop = clamp(origin.y + dy - parentRect.top, minTop, maxTop);
    ...
    avatarTrigger?.addEventListener('click', (e) => {
        ...
        avatarMenu.setAttribute('aria-hidden', String(!isOpen));
    ...
    document.addEventListener('click', (e) => {
        if (!avatarMenu.contains(e.target) && !(avatarTrigger && avatarTrigger.contains(e.target))) {
            avatarMenu.classList.remove('open');
    ...
    function createSoundItem(sound) {
        ...
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            // Determinar si el sonido debe reproducirse o pausarse con un solo clic
            const shouldPlay = !activeAudios[sound.id] || (activeAudios[sound.id] && activeAudios[sound.id].player.paused);
            toggleSound(sound.id, sound.name, sound.file, shouldPlay);
        });
    ...
    volGroup.addEventListener('click', (e) => {
        ...
        // Helper to update the visual filled portion of the range control
        function updateRangeFill(r) {
            const v = Math.max(0, Math.min(1, parseFloat(r.value) || 0));
            const pct = (v * 100).toFixed(2);
            r.style.backgroundSize = `${pct}% 100%`;
        }
    ...
    function updatePlaylistUI() {
        ...
        // Mostrar/ocultar el área del reproductor completo según si hay archivos
        const playerDisplay = $('#media-local-content .local-player-display');
        if (localPlaylistFiles.length === 0) {
            // Ocultar controles y lista, mostrar sólo el botón de subir
            if (playerDisplay) playerDisplay.style.display = 'none';
            if (playlistEl) playlistEl.style.display = 'none';
        } else {
            if (playerDisplay) playerDisplay.style.display = '';
            if (playlistEl) playlistEl.style.display = '';
        ...
        if (deleteLocalTracksBtn) deleteLocalTracksBtn.style.display = localPlaylistFiles.length > 0 ? 'inline-flex' : 'none';
    }
    ...
    // Aseguramos que la barra del slider local también muestre el progreso visual correctamente
    function updateRangeFillStatic(r) {
        const v = Math.max(0, Math.min(1, parseFloat(r.value) || 0));
        const pct = (v * 100).toFixed(2);
        r.style.backgroundSize = `${pct}% 100%`;
    }
    ...
    if (volumeSlider) {
        // Inicializar el fill visual
        updateRangeFillStatic(volumeSlider);
        volumeSlider.addEventListener('input', (e) => {
            localPlayer.volume = e.target.value;
            updateRangeFillStatic(e.target);
        });
    }
    ...
    function saveTasks() {
        try {
            ...
            console.error('No se pudo guardar la lista de tareas:', e);
        }
    }

    // Añadir nueva tarea desde el input / botón
    addTaskBtn?.addEventListener('click', (e) => {
        ...
        // Añadir confirmación visual: desplazamiento suave + flash en la nueva tarea
        try {
            const lastItem = taskList?.lastElementChild;
            if (lastItem) {
                lastItem.classList.add('task-added');
                lastItem.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                setTimeout(() => lastItem.classList.remove('task-added'), 800);
            }
        } catch (err) {
            console.warn('No se pudo animar la tarea añadida:', err);
        }
    });

    // Permitir añadir la tarea con Enter
    ...
    clearCompletedBtn?.addEventListener('click', () => {
        ...
        if (tasks.length === before) return; // nada que limpiar
        saveTasks();
        renderTasks();
    });
    ...
    function loadProfileForMenu() {
        ...
        // Mostrar nombre también en la parte superior junto al avatar
        if (topbarUsername) topbarUsername.textContent = data.accountName || 'Usuario';
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        (function init() {
            ...
            // Asegurar que la UI del reproductor local refleje la existencia (o no) de pistas subidas
            try { updatePlaylistUI(); } catch (e) { /* noop si no está disponible aún */ }
