...
function loadFondosGlobalBlobs() {
  fetch('obtenerFondosGlobalBlobs.php')
    ...
    // Lógica para subir fondo animado propio
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!uploadBackgroundBtn || !backgroundFileInput) {
            console.error('No se encontraron los elementos upload-background-btn o background-file-input en el DOM.');
            return;
        }

        // Asegurarse de que el botón abra la ventana de selección de archivos
        uploadBackgroundBtn.addEventListener('click', () => {
            console.log('Botón clickeado, abriendo ventana de selección de archivos.');
        ...
        backgroundFileInput.addEventListener('change', (e) => {
            ...
            if (!['video/mp4', 'image/gif'].includes(file.type)) {
                alert('Solo se permiten archivos MP4 o GIF.');
                return;
            }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Opcional: Recargar la galería de fondos
                    loadFondosGlobalBlobs();
                } else {
...
// --- Subir Sonido Propio ---
const uploadSoundBtn = document.querySelector('#upload-sound-btn');
...
if (uploadSoundBtn && soundFileInput) {
    ...
    soundFileInput.addEventListener('change', async (event) => {
        ...
        try {
            const response = await fetch('subirSonidoUsuario.php', {
                method: 'POST',
                body: formData
            });
...
// Cargar fondos de usuario
document.addEventListener('DOMContentLoaded', () => {
    const userBackgroundGallery = document.getElementById('user-background-gallery');

    if (userBackgroundGallery) {
        // Función para cargar los fondos del usuario
        const cargarFondosUsuario = async () => {
            try {
                const response = await fetch('obtenerFondosUsuario.php');
                ...
                if (data.success) {
                    ...
                    data.fondos.forEach(fondo => {
                        ...
                        fondoItem.classList.add('fondo-item');
                        fondoItem.textContent = fondo.nombre;
                        userBackgroundGallery.appendChild(fondoItem);
                    });
                } else {
                    console.error('Error al cargar los fondos del usuario:', data.message);
                }
            } catch (error) {
                console.error('Error al realizar la solicitud para cargar fondos del usuario:', error);
            }
        };

        // Cargar los fondos al iniciar
        cargarFondosUsuario();
    }
});

// Function to load user backgrounds dynamically
function loadUserBackgrounds() {
    fetch('obtenerFondosUsuario.php')
        .then(response => {
            if (!response.ok) {
                throw new Error('Error al obtener los fondos del usuario');
            }
        ...
        .then(data => {
            if (data.success) {
                ...
                data.fondos.forEach(fondo => {
                    ...
                    fondoElement.className = 'background-item';
                    fondoElement.style.backgroundImage = `url(verBlobGlobal.php?tipo=fondo&id=${fondo.id})`;
                    fondoElement.title = fondo.nombre;

                    // Add click event to apply background
                    fondoElement.addEventListener('click', () => {
                        applyBackground(`verBlobGlobal.php?tipo=fondo&id=${fondo.id}`, 'user');
                    });

                    gallery.appendChild(fondoElement);
                });
            } else {
        ...
        .catch(error => {
            console.error('Error al cargar los fondos del usuario:', error);
        });
}
})();